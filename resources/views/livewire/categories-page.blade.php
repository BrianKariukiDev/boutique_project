<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto bg-slate-400">
            ...
            <div class="flex flex-wrap mb-24 -mx-3">
                <div class="w-full pr-2 lg:w-1/4 lg:block">
                            ...
                            @foreach ($categories as $category)
                                <li class="mb-4">
                                    <label for="{{$category->slug}}" class="flex items-center dark:text-gray-400 ">
                                        <input type="checkbox" id="{{$category->slug}}" class="w-4 h-4 mr-2" wire:model.live='selectedCategories' value="{{$category->id}}">
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <li class="mb-4">
                                <label for="{{$brand->slug}}" class="flex items-center dark:text-gray-300">
                                    <input type="checkbox" id="{{$brand->slug}}" class="w-4 h-4 mr-2" wire:model.live='selectedBrands' value="{{$brand->id}}">
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
                        <div>
                        {{--<input type="number" wire:model.live='priceRange' min="1" class="bg-white text-black"/>--}}
                            <input type="range"
                                class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer"
                                max="200000" wire:model.live='priceRange' step="100">
                            <div class="flex justify-between ">
                                <span class="inline-block text-lg font-bold text-blue-400 ">{{Number::currency(0,'KSHS')}}</span>
                                <span class="inline-block text-lg font-bold text-blue-400 ">{{Number::currency($priceRange, 'KSHS')}}</span>
                            </div>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div class="relative bg-gray-200">
                                        <a href="/products/{{ $product->slug }}" class="">
                                            <img src="{{ url('storage', $product->images[0]) }}"
                                                alt="{{ $product->name }}" class="object-cover w-full h-full mx-auto ">
                                        </a>
                                    </div>
                                    <div class="p-3 ">
                                        ...
                                        <p class="text-lg ">
                                            <span
                                                class="text-green-600 dark:text-green-600">{{ Number::currency($product->sale_price, 'KSHS') }}</span>
                                        </p>
                                    </div>
                                    <div class="flex justify-center p-4 border-t border-gray-300 dark:border-gray-700">

                                        <a wire:click.prevent="addToCart({{ $product->id }})"
                                            class="text-gray-500 flex items-center space-x-2 dark:text-gray-400 hover:text-red-500 hover:cursor-pointer dark:hover:text-red-300">
